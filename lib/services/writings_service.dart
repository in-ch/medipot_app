import 'dart:convert';
import 'package:http/http.dart' as http;

class WritingsService {
  static Future<Map<String, dynamic>> getWritings(
      String tag, String text) async {
    final url =
        Uri.parse('http://localhost:4000/writing/list?text=$text&tag=$tag');
    final headers = {
      'Authorization':
          'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..an3AW7J8KW6hLihah-aVK9PzIgxkYeFJrRFmzYsdOIo'
    };

    final response = await http.get(url, headers: headers);
    if (response.statusCode == 200) {
      return jsonDecode(response.body);
    } else {
      throw Exception('Failed to writings fetch list');
    }
  }

  static Future<Map<String, dynamic>> getWriting(int no) async {
    final url = Uri.parse('http://localhost:4000/writing/detail?no=$no');
    final headers = {
      'Authorization':
          'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..an3AW7J8KW6hLihah-aVK9PzIgxkYeFJrRFmzYsdOIo'
    };

    final response = await http.get(url, headers: headers);
    if (response.statusCode == 200) {
      return jsonDecode(response.body);
    } else {
      throw Exception('Failed to get writing API');
    }
  }
}
